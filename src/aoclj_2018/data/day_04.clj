(ns aoclj-2018.data.day-04
  (:require [clojure.string :as str]
            [clojure.edn :as edn]))

(defn action-data
  [command-str]
  (when-not (seq command-str)
    (throw (Exception. "Command not provided.")))
  (condp re-matches command-str
    #".*begin.*" {:id (re-find #"\d+" command-str) :val 1}
    #".*wakes up" {:val 1}
    #".*falls asleep" {:val 0}
    nil))
...
(defn entry-item
  [input-str]
  (if (seq input-str)
    (let [[_ date-str time-str command-str] (re-find #"\[(.*)\s(.*)\]\s(.*)" input-str)]
      {:date (into [] (str/split date-str #"-"))
       :time (into [] (str/split time-str #"\:"))
       :action (action-data command-str)})))

(defn log-entry [log id {:keys [date time action]}]
  (assoc-in log [id time] (get action :val)))

(defn build-log [input]
  (when (empty? input) (throw (Exception. "No input provided.")))
  (loop [[entry-str & others] input
         current-id nil
         log {}]
    (if-let [entry (entry-item entry-str)]
      ;; If entry has an ID use that, otherwise use current-id
      (let [id (or (get-in entry [:action :id]) current-id)]
        (recur others id (log-entry log id entry)))
      log)))

(comment
  ;;0000000000111111111112222
  ;;0123456789012345678901234
  ;;*
  ;;{:1223 {[0 0]}}
  ;; (assoc-in {} [1234 [0 0]] "*")

  (re-matches #".*begin.*" "Guard #3529 begins shift")
  (action-data "Guard #3529 begins shift")
  (action-data "wakes up")
  (action-data "falls asleep")
  (entry-item "")
  (log-entry {} {:time [0 0] :action {:val 1}})
  (let [input ["[1518-03-18 00:03] Guard #3529 begins shift"
               "[1518-04-26 00:55] wakes up"
               "[1518-04-22 00:56] falls asleep"
               "[1518-03-18 00:12] Guard #4006 begins shift"
               "[1518-04-26 00:22] wakes up"
               "[1518-04-08 24:33] falls asleep"]]
    (build-log input))
  )

(def input ["[1518-03-18 00:03] Guard #3529 begins shift"
            "[1518-04-26 00:55] wakes up"
